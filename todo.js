/*
js run on local machine as well as browser(on client ansd server both)
but when we embadded inside html it rurn on the browser(browser console)
*/
let todos = [];
function handleToDoFormSumbit(){
    console.log("Form Submitted");
    // 1. get the text box.
    // document refer to the dom(Document Object Model)
    const textTodo = document.getElementById("txtTodo")
    //console.log(textTodo);

    // 2. Get data/value from text box.
    const todoTextValue = textTodo.value;
    textTodo.value="";
...
//     ul.innerHTML = ul.innerHTML+li;   
    ...
    //5. find the ul where we need to add created li  
    // Find the ul where we need to add created li.
    const ul = document.getElementById("todoList");
   // li.appendChild(label).appendChild(ebtn).appendChild(dbtn);

    // apppend li to innerHTML of ul.
    ul.appendChild(li);     // ul.innerHTML = ul.innerHTML+li;  
}
function handleEditClick(event){
    ...
    txtBox.classList.add("form-control");
               /////OR/////
  //  const txtBox = "<input value'"+label.innerText+"' class = 'form-control' type = 'text' id = 'txtBoxToUpdate' />" ;
...
function updateToDo(event){
    ...
    const textb = document.getElementById("txtbUpdate").value;
...
function deleteToDo(event){
     // 1. get id of delete button.
     const button = document.getElementById(event.tardet.id);
     // 2. get parent li.
